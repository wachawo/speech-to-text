...
Open `http://<host>:8080`. **TRANSCRIBE** has three sources. **FILE** uploads an audio file. **DEVICE** transcribes live from an audio input: a microphone or a headset, a `Monitor of ...` source on Linux that carries whatever plays through the speakers or headphones, or, in Chromium browsers, `Tab or screen audio` for what plays in a browser tab, or in the whole system where the OS allows it. **STREAM** takes the address of a stream or a remote file - internet radio, HLS, RTMP, RTSP, SRT - and the server reads it itself. Either way the result appears under the form as plain text or, with diarization enabled, as one block per phrase with its speaker and time, each speaker in its own colour and the phrases where two people talked at once marked; a live phrase appears about a second after the speaker pauses. The result can be copied or downloaded as TXT or JSON, and each source keeps its last result while you look at another screen. FILE also has a **Turns** mode, `POST /api/diarize` on its own: who spoke when, drawn as a timeline with a lane per speaker and the stretches where two people talk at once marked. **MODELS** shows what `GET /api/models` reports. When `STT_TOKENS` is set, the UI asks for a token once and keeps it in the browser.
...
**Long recordings go through a job.** `POST /api/jobs` takes the same body shapes as `/api/stt`, up to `JOB_MAX_CONTENT_LENGTH_MB` (1 GB), plus a `mode`: `text` (the default), `speakers` (the `/api/transcript` result) or `turns` (the `/api/diarize` result). It answers `202` at once with the job's id and a `Location` to poll:
...
`status` goes `queued` (with its `position`), `running`, then `done` with the `result`, or `failed` with an `error` category. `GET /api/jobs` lists them and `DELETE /api/jobs/<id>` removes one that is not running. A job works through the file in pieces of about a minute, cut at pauses, and gives the model back between them, so the short requests keep being answered while it runs: a 30-minute recording took 107 s on the deployment GPU, and phone-call requests sent during it took 10 s at worst. Diarization runs across the whole file in the model's streaming mode, so a speaker keeps one number from the first minute to the last. Jobs are files under `JOBS_DIR`, so they survive a restart, and finished ones are removed after `JOB_RETENTION_HOURS`.
...
3. The client sends raw PCM - signed 16-bit little-endian, mono, 16 kHz - as binary messages of any size, at the pace it is recorded, and `{"type": "stop"}` when it is done. A file sent faster than real time falls behind by design and loses phrases (see `skipped` below); a file belongs in a job.
...
The audio can also come from somewhere else. With `"source": "url", "url": "https://..."` in the start message the client sends no audio at all: the server reads the stream at that address through ffmpeg until it ends or the client sends `stop`. Internet radio, HLS, RTMP, RTSP and SRT work; the scheme must be `http`, `https`, `rtmp`, `rtmps`, `rtsp` or `srt`. A live source's backlog is taken at once and the rest at the source's own pace, so a remote file arrives the way a broadcast would.

Because this makes the server fetch an address a client chose, it is fenced in. ffmpeg may use network protocols only, so neither a URL nor a playlist can make it read a local file, and nothing may make it listen: SRT `listener` and `rendezvous` modes and any `listen` parameter are refused. A page on another site cannot start one (`Forbidden`): browsers apply no CORS to websockets, so the socket checks that the page's origin is this host, or is listed in `CORS_ORIGINS`. At most four URL sources run at once (`Service Unavailable` beyond that), and the log records each address without its credentials or query. It can still reach hosts on the server's own network, which is the point for a camera and the reason to set `STT_TOKENS` on a server others can reach.
...
Uploads are capped at `MAX_CONTENT_LENGTH_MB` (10 MB for the bare server, 100 MB in the Docker setup); a larger body returns `413`.
...
| `JOBS_DIR`              | `recs/jobs`             | where jobs keep their upload, record and result     |
| `JOB_MAX_CONTENT_LENGTH_MB` | `1024`              | max upload size of a job in MB                      |
| `JOB_RETENTION_HOURS`   | `24`                    | finished jobs older than this are removed           |
