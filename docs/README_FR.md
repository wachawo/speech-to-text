...
Ouvrez `http://<host>:8080`. **TRANSCRIBE** propose trois sources. **FILE** envoie un fichier audio. **DEVICE** transcrit en direct depuis une entrée audio : un microphone ou un micro-casque, une source `Monitor of ...` sous Linux qui transporte tout ce qui est joué par les haut-parleurs ou les écouteurs, ou, dans les navigateurs Chromium, `Tab or screen audio` pour ce qui est joué dans un onglet du navigateur, ou dans tout le système lorsque le système d'exploitation le permet. **STREAM** prend l'adresse d'un flux ou d'un fichier distant - radio sur Internet, HLS, RTMP, RTSP, SRT - et le serveur la lit lui-même. Dans tous les cas, le résultat apparaît sous le formulaire en texte brut ou, avec la diarisation activée, sous forme d'un bloc par phrase avec son locuteur et son horodatage, chaque locuteur dans sa propre couleur et les phrases où deux personnes ont parlé en même temps signalées ; une phrase en direct apparaît environ une seconde après que le locuteur a marqué une pause. Le résultat peut être copié ou téléchargé en TXT ou en JSON, et chaque source conserve son dernier résultat pendant que vous consultez un autre écran. FILE propose aussi un mode **Turns**, `POST /api/diarize` seul : qui a parlé et quand, dessiné sous forme de frise chronologique avec une ligne par locuteur, les passages où deux personnes parlent en même temps étant signalés. **MODELS** affiche ce que renvoie `GET /api/models`. Lorsque `STT_TOKENS` est défini, l'interface demande un jeton une seule fois et le conserve dans le navigateur.
...
L'audio peut aussi venir d'ailleurs. Avec `"source": "url", "url": "https://..."` dans le message de démarrage, le client n'envoie aucun audio : le serveur lit le flux à cette adresse via ffmpeg jusqu'à ce qu'il se termine ou que le client envoie `stop`. La radio sur Internet, HLS, RTMP, RTSP et SRT fonctionnent ; le schéma doit être `http`, `https`, `rtmp`, `rtmps`, `rtsp` ou `srt`. Ce qu'une source en direct a déjà en réserve est pris d'un coup, et le reste au rythme propre de la source, de sorte qu'un fichier distant arrive comme le ferait une diffusion.

Comme cela amène le serveur à récupérer une adresse choisie par un client, c'est strictement encadré. ffmpeg ne peut utiliser que des protocoles réseau, de sorte que ni une URL ni une playlist ne peut lui faire lire un fichier local, et rien ne peut le mettre en écoute : les modes SRT `listener` et `rendezvous` et tout paramètre `listen` sont refusés. Une page d'un autre site ne peut pas en démarrer une (`Forbidden`) : les navigateurs n'appliquent pas CORS aux WebSockets, le socket vérifie donc que l'origine de la page est cet hôte ou figure dans `CORS_ORIGINS`. Au plus quatre sources URL tournent en même temps (`Service Unavailable` au-delà), et le journal enregistre chaque adresse sans ses identifiants ni sa chaîne de requête. Une source URL peut toujours atteindre des hôtes du réseau du serveur lui-même, ce qui est tout l'intérêt pour une caméra et la raison de définir `STT_TOKENS` sur un serveur que d'autres peuvent atteindre.
...
Les envois sont limités à `MAX_CONTENT_LENGTH_MB` (10 Mo pour le serveur seul, 100 Mo dans l'installation Docker) ; un corps plus grand renvoie `413`.
