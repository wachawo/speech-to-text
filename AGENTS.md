...
- **Whisper's own confidence cannot catch its hallucinations; a voice detector does.** On tone, music, noise, ringback and even digital silence Whisper returns subtitle credits (a Russian "subtitles by DimaTorzok" line, "to be continued...") with `no_speech_prob` 0.00 and the best log-probabilities of the run, so thresholds on those numbers remove nothing. `libs/speech_gate.py` runs Silero VAD (threshold 0.35, 100 ms minimum speech - tuned on a corpus so that no segment of real speech fell under the 0.3 share) and drops segments mostly outside speech, plus whole-segment credit lines no person says, which covers music with vocals. A detector failure fails open and is logged once. Tests run with `SPEECH_GATE` off (autouse fixture) because CI has no silero-vad; the gate's own tests replace `detect_speech`. The detector is imported inside functions: it pulls in torch, which must not load before `fork()`.
- **With diarization, a live phrase is also cut at a speaker change.** `libs/live.py::split_at_handover` runs after each idle catch-up and asks `diarize.find_handover` for the first turn of another speaker that has lasted 0.4 s while the phrase's speaker was silent; overlap is not a handover and stays one phrase with `overlap` set.
...
- **The GPU image asserts its own torch.** `Dockerfile` installs a `+cu130` wheel and then runs further unconstrained resolutions; anything depending on torch can replace it, and the failure surfaces much later as cuBLAS and cuDNN errors that read like a driver problem. A `RUN python3 -c "assert '+cu130' in torch.__version__"` turns that into a build failure. Do not remove it, and add the same shape of check for any new pin that matters. The same Dockerfile checks torch's compiled arch flags for GB10 (`sm_120` or its `compute_120` PTX; the build has no `sm_121`), read from `torch._C._cuda_getArchFlags()` because `get_arch_list()` is empty without a GPU at build time. Every install step runs with `-c constraints.txt`, the versions of the last verified image; regenerate it with `make constraints` after a dependency change is built and checked, and leave torch, the CUDA wheels and transformers out of it. The uv image is pinned (`uv:0.12.19`): a moving `latest` threw away every later layer's cache.
...
- **A URL source makes the server fetch what a client names.** `libs/url_source.py` owns it, and every rule there leans towards refusing. One normalised string is both what is checked and what ffmpeg runs (urlsplit silently drops tabs and newlines ffmpeg would keep, so checking a cleaned copy is how a URL passes and then does something else). ffmpeg gets `-protocol_whitelist` with network protocols only, because a playlist fetched over http can point at `file:` or `concat:`; nothing may make it listen (SRT listener/rendezvous, `listen`); a browser page may start one only from this host's origin, since browsers apply no CORS to websockets; at most `MAX_URL_SESSIONS` run at once; the log gets the address without credentials or query. RTSP takes `-timeout`, every other scheme `-rw_timeout`: ffmpeg aborts on an input option nobody consumed. stderr is drained while ffmpeg runs - read after exit, a noisy source fills the pipe and freezes the session. `-readrate 1` with an initial burst, never `-re`, which paces a live source's backlog into permanent lag. ffmpeg is killed when the session ends, whichever way it ends.
- **Browser capture runs at the device's own rate and downsamples itself.** `www/js/capture.js` opens the device (echo cancellation and noise suppression off: they damage a loopback or a second voice) or `getDisplayMedia` for tab or system sound, and an AudioWorklet averages it down to 16 kHz mono PCM16 in 100 ms frames. An AudioContext created at 16 kHz would be simpler, and Firefox refuses to connect a stream of another rate into it. Devices exist only on a secure page, which is what the https listener of `stt_www` is for.
- **Containers start as root, then drop.** `entrypoint.sh` runs as root, chowns the bind-mounted `/opt/models`, `/opt/logs`, `/opt/recs` (host-owned mounts the build-time chown cannot reach) and `exec`s the command via `setpriv` as `STT_UID`:`STT_GID` when set - the host user, so those directories stay writable to them - and as the image's `stt` user otherwise. Such a uid owns nothing in the image and its `$HOME` is `/root`, which is why the Dockerfiles point `NUMBA_CACHE_DIR` and `XDG_CACHE_HOME` at `/tmp`: numba, under librosa, refuses to import without a writable cache, and that took the diarizer down on the first try. Without it the first `whisper.load_model` dies with `PermissionError`.
...
make typecheck     # mypy
make constraints   # freeze the verified GPU image's dependency versions into constraints.txt
make tag-rollback  # tag both running images as rollback-<rev> before a deploy
...
- Speech gate: `SPEECH_GATE` (default true; needs silero-vad, which the `whisper` extra and `requirements.txt` bring)
...
- Container (read by `entrypoint.sh` and Hugging Face, not by `libs/config.py`): `STT_UID`, `STT_GID`, `HF_HUB_OFFLINE`
...
- `/api/stream` (websocket) - live transcription: a JSON `start` message, then raw 16 kHz mono PCM16 as binary messages, then `stop` - or `"source": "url"` and no audio, the server pulling it with ffmpeg; the server answers `ready`, a `segment` per phrase (`{id, start, end, text, speaker, overlap}`), `progress`, `skipped` (seconds dropped because the queue held more than `MAX_BACKLOG_SECONDS` of audio), and `done`. Errors are one `{"type": "error", "error", "request_id"}` and a close. A browser sends its token in the start message, other clients may use the `Authorization` header.
