...
- **A URL source makes the server fetch what a client names.** `"source": "url"` runs ffmpeg on that address. The scheme is checked against `URL_SCHEMES` and ffmpeg gets `-protocol_whitelist` with network protocols only, because a playlist fetched over http can point at `file:` or `concat:` entries; neither check stops it from reaching hosts on the server's own network, which is why the README tells anyone exposing it to set `STT_TOKENS`. ffmpeg is killed when the session ends, whichever way it ends.
